...
let revenueView = 'calendar'; // 'calendar' | 'list' | 'summary'
...
function getPrice(timer) {
  return getElapsed(timer) * PRICE_PER_SECOND;
}
...
function commitRunningElapsed(timer) {
  if (timer.status === 'running' && timer.lastStartedAt) {
    timer.accumulatedElapsed += (Date.now() - timer.lastStartedAt) / 1000;
    timer.lastStartedAt = Date.now();
  }
}
...
function getMonthRevenue(year, month) {
  ...
  for (let d = 1; d <= days; d++) {
    const key = getMonthDateKey(year, month, d);
    total += revenue[key] || 0;
  }
  return total;
}

function getMonthDateKey(year, month, day) {
...
function getWeekTotal(year, month, days) {
  return days.reduce((sum, day) => sum + (revenue[getMonthDateKey(year, month, day)] || 0), 0);
}
      ...
      <div class="time-adjust">
        ...
        <button class="btn btn-ghost btn-sm" onclick="adjustTime(timers.find(t=>t.id==='${timer.id}'), 5)">+5د</button>
	      ...
	      <div class="countdown-edit">
	        <label>تعديل الوقت الخارجي</label>
	        <div>
	          <input type="number" min="1" max="600" value="${Math.round(timer.initialDuration / 60)}"
	                 ondragstart="event.stopPropagation()" onclick="event.stopPropagation();this.select()"
	                 onchange="setCountdownDuration(timers.find(t=>t.id==='${timer.id}'), this.value)">
	          <span>دقيقة</span>
	        </div>
	      </div>` : ''}
	    </div>`;
...
function setCountdownDuration(timer, minutes) {
  if (!timer || timer.type !== 'countdown') return;
  commitRunningElapsed(timer);
  const nextDuration = Math.max(60, (parseInt(minutes, 10) || 15) * 60);
  timer.initialDuration = nextDuration;
  if (timer.accumulatedElapsed >= nextDuration) {
    timer.accumulatedElapsed = nextDuration;
    timer.status = 'stopped';
    timer.lastStartedAt = null;
    timer.finished = true;
  } else {
...
function renderCalendar() {
  ...
  const selectedRevenue = revenue[selectedKey] || 0;

  return `<div class="revenue-panel active">
    <div class="cal-nav">
      <button class="btn btn-ghost btn-sm" onclick="moveMonth(-1)">→ الشهر السابق</button>
      <span class="cal-month-name">${monthNames[month]} ${year} — ${formatPrice(monthTotal)}</span>
      <button class="btn btn-ghost btn-sm" onclick="moveMonth(1)">الشهر التالي ←</button>
      <button class="btn btn-accent btn-sm" onclick="goToday()">اليوم</button>
    </div>

    <div class="summary-section revenue-totals">
      <div class="summary-row"><span class="summary-label">الدخل اليومي</span><span class="summary-value">${formatPrice(getTodayRevenue())}</span></div>
      <div class="summary-row"><span class="summary-label">الدخل الأسبوعي</span><span class="summary-value">${formatPrice(getWeekRevenue())}</span></div>
      <div class="summary-row"><span class="summary-label">الدخل الشهري</span><span class="summary-value">${formatPrice(monthTotal)}</span></div>
    </div>

    <div class="revenue-tabs">
      ${['calendar','list','summary'].map(v => `<button class="nav-tab ${revenueView===v?'active':''}" onclick="switchRevenueView('${v}')">${v === 'calendar' ? '📅 تقويم' : v === 'list' ? '📋 قائمة' : '📊 ملخص'}</button>`).join('')}
    </div>

    ${revenueView === 'calendar' ? renderCalendarGrid(year, month, daysInMonth, firstDay, dayNames, today, selectedDay) : ''}
    ${revenueView === 'list' ? renderRevenueList(year, month, daysInMonth, monthNames[month]) : ''}
    ${revenueView === 'summary' ? renderRevenueSummary(year, month, monthNames[month]) : ''}

    <aside class="day-detail side-detail">
      <div class="day-detail-nav">
        <button class="btn btn-ghost btn-sm" onclick="navigateDay(-1)">→</button>
        <strong>${selected.getDate()} ${monthNames[selected.getMonth()]} ${selected.getFullYear()}</strong>
        <button class="btn btn-ghost btn-sm" onclick="navigateDay(1)">←</button>
      </div>
      <div class="summary-row"><span class="summary-label">إيراد هذا اليوم</span><span class="summary-value">${formatPrice(selectedRevenue)}</span></div>
      <div class="summary-row"><span class="summary-label">مفتاح التخزين</span><span class="summary-value">${selectedKey}</span></div>
      <button class="btn btn-accent btn-sm" onclick="goToday()" style="margin-top:10px;">رجوع إلى اليوم</button>
    </aside>
  </div>`;
}

function renderCalendarGrid(year, month, daysInMonth, firstDay, dayNames, today) {
  let html = `<div class="calendar-layout"><div class="calendar-grid calendar-grid-weeks">`;
  html += `<div class="cal-header week-head">أسبوع</div>`;
  dayNames.forEach(d => { html += `<div class="cal-header">${d}</div>`; });

  let currentDay = 1;
  let weekNum = 1;
  while (currentDay <= daysInMonth) {
    html += `<div class="cal-week-num">${weekNum}</div>`;
    for (let dow = 0; dow < 7; dow++) {
      if ((weekNum === 1 && dow < firstDay) || currentDay > daysInMonth) {
        html += `<div class="cal-day cal-empty"></div>`;
      } else {
        const dateKey = getMonthDateKey(year, month, currentDay);
        ...
        html += `<div class="cal-day ${isToday?'today':''} ${isSelected?'selected':''} ${dayRevenue > 0?'has-revenue':''}" onclick="selectDay(${currentDay})">
          <span>${currentDay}</span>
          ${dayRevenue > 0 ? `<span class="day-amount">${dayRevenue.toFixed(0)} دج</span>` : `<span class="day-amount muted">0</span>`}
        </div>`;
        currentDay++;
      }
    }
...
function renderRevenueList(year, month, daysInMonth, monthName) {
  let html = `<div class="summary-section month-list"><h4>${monthName} ${year}</h4>`;
  for (let d = 1; d <= daysInMonth; d++) {
    const key = getMonthDateKey(year, month, d);
    html += `<button class="revenue-list-row" onclick="selectDay(${d})">
      <span>اليوم ${d}</span>
      <span>${formatPrice(revenue[key] || 0)}</span>
    </button>`;
  }
  html += `<div class="summary-row month-total"><span class="summary-label">مجموع الشهر</span><span class="summary-value">${formatPrice(getMonthRevenue(year, month))}</span></div></div>`;
  return html;
}

function renderRevenueSummary(year, month, monthName) {
  const weeks = getMonthWeeks(year, month);
  let html = `<div class="summary-section month-summary"><h4>ملخص ${monthName} ${year}</h4>`;
  weeks.forEach((days, idx) => {
    html += `<div class="week-card"><div class="summary-row"><span class="summary-label">الأسبوع ${idx + 1}</span><span class="summary-value">${formatPrice(getWeekTotal(year, month, days))}</span></div>`;
    days.forEach(day => {
      const key = getMonthDateKey(year, month, day);
      html += `<button class="revenue-list-row compact" onclick="selectDay(${day})"><span>اليوم ${day}</span><span>${formatPrice(revenue[key] || 0)}</span></button>`;
    });
    html += `</div>`;
  });
  html += `<div class="summary-row month-total"><span class="summary-label">المجموع الشهري الكامل</span><span class="summary-value">${formatPrice(getMonthRevenue(year, month))}</span></div></div>`;
  return html;
}

function getSelectedDate() {
  const day = selectedDay || new Date().getDate();
  const max = new Date(calendarYear, calendarMonth + 1, 0).getDate();
  return new Date(calendarYear, calendarMonth, Math.min(day, max));
}
...
function moveMonth(delta) {
  ...
  const max = new Date(calendarYear, calendarMonth + 1, 0).getDate();
  selectedDay = Math.min(selectedDay || 1, max);
  render();
}
